using Omics

using Test: @test

# ----------------------------------------------------------------------------------------------- #

# ---- #

for (nu_, re) in (
    ([1.0], Omics.Palette.bwr),
    ([1], Omics.Palette.MO),
    ([1, 2], Omics.Palette.BI),
    ([1, 2, 3], Omics.Palette.CA),
)

    @test Omics.Palette.pick(nu_) === re

end

# ---- #

const RG_ = Omics.Palette.make(["#ff0000", "#00ff00", "#0000ff"])

for (nu, re) in (
    # Indexed.
    (1, "#ff0000ff"),
    (2, "#00ff00ff"),
    (3, "#0000ffff"),
    # Normalized between 0 and 1.
    (-Inf, "#ff0000ff"),
    (-0.1, "#ff0000ff"),
    (0.0, "#ff0000ff"),
    (0.01, "#fa0500ff"),
    (0.5, "#00ff00ff"),
    (0.99, "#0005faff"),
    (1.0, "#0000ffff"),
    (1.1, "#0000ffff"),
    (Inf, "#0000ffff"),
    # Normalized between the extrema.
    ([NaN], ["#00ff00ff"]),
    ([-1], ["#00ff00ff"]),
    ([0], ["#00ff00ff"]),
    ([4], ["#00ff00ff"]),
    (1:3, ["#ff0000ff", "#00ff00ff", "#0000ffff"]),
    (1:4, ["#ff0000ff", "#55aa00ff", "#00aa55ff", "#0000ffff"]),
)

    @test Omics.Palette.color(nu, RG_) == re

end

# ---- #

for (he_, re) in (
    (["#000001"], [(0, "#000001ff"), (1, "#000001ff")]),
    (["#000001", "#000002"], [(0.0, "#000001ff"), (1.0, "#000002ff")]),
    (
        ["#000001", "#000002", "#000003"],
        [(0.0, "#000001ff"), (0.5, "#000002ff"), (1.0, "#000003ff")],
    ),
    (
        ["#000001", "#000002", "#000003", "#000004"],
        [
            (0.0, "#000001ff"),
            (0.3333333333333333, "#000002ff"),
            (0.6666666666666666, "#000003ff"),
            (1.0, "#000004ff"),
        ],
    ),
    (
        ["#000001", "#000002", "#000003", "#000004", "#000005"],
        [
            (0.0, "#000001ff"),
            (0.25, "#000002ff"),
            (0.5, "#000003ff"),
            (0.75, "#000004ff"),
            (1.0, "#000005ff"),
        ],
    ),
    (
        ["#000001", "#000002", "#000003", "#000004", "#000005", "#000006"],
        [
            (0.0, "#000001ff"),
            (0.2, "#000002ff"),
            (0.4, "#000003ff"),
            (0.6, "#000004ff"),
            (0.8, "#000005ff"),
            (1.0, "#000006ff"),
        ],
    ),
)

    @test Omics.Palette.fractionate(Omics.Palette.make(he_)) == re

end
